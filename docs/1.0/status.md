...
- Notebook statuslines show kernel truth separately from transport and label disconnected cached state as last-known. Never-connected notebooks are gray, mode:cell has a purple background badge, and output labels omit .vipynb. Output statuslines expose monotonic editor-local IDs for counted G/Q and backslash g/q targeting; stale IDs cannot close replacements. Notebook-local backslash actions cover editing, submission, history, parking and lifecycle. See [ADR 0042](adr/0042-statuslines-and-output-addressing.md).
