...
- Internal copy/open request and delivery helpers transfer plugin-selected UTF-8 content over HTTP. Public `JusiCopy`/`JusiOpen` commands were withdrawn: application-initiated actions are the intended workflow. Exports are bounded independent snapshots, with no remote filesystem path or writeback. See [ADR 0035](adr/0035-editor-actions-transfer-content-over-http.md). Terminal-application-initiated actions/work leases remain unimplemented; the [delivery proposal](architecture/backend-driven-editor-actions.md) records the legacy review and intended flow.
