...
- Bundled `%%vd` now uses ordinary plugin discovery, kernel-expression snapshots, an isolated worker and VisiData terminal, with application-driven `zY` copy and Ctrl-O open. Python containers and pandas tables are supported within bounded data snapshots; no-op legacy followups are not advertised. VisiData is available through the optional `vd` extra. See [ADR 0037](adr/0037-bundled-visidata-provider.md), the [usage guide](architecture/bundled-vd.md), and [Incident 0014](incidents/0014-embedded-visidata-startup-and-editor-actions.md). Manual review is next.
