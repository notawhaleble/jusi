...
- Structural extmarks cover the delimiter text. Deleting a delimiter invalidates its anchor; undo or later insertion cannot restore the retired cell identity.
...
A structural or ambiguous edit reparses from the affected opener to the next surviving opener. The next opener is both the grammar recovery point and the reconciliation boundary. Only cells in that linked segment are retired, reused, or inserted.
