...
def test_application_action_shared_contract():
    for name in ["application-editor-action", "application-editor-result"]:
    ...
    for name in ["application-action-wrong-kind", "application-action-path"]:
...
@pytest.mark.parametrize("validator,valid,invalids", [
    ...
    (validate_editor_action_fetch, "editor-action-fetch", ["editor-action-fetch-expiry", "editor-action-fetch-kind"]),
