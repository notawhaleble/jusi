...
function M.run()
  ...
  local ok, failure = xpcall(function()
    ...
    -- Enter's real-execution branch cleans completed output from another cell.
    local previous_output = assert(session.presentation:buffer_for_cell(cell_id))
    vim.api.nvim_buf_set_lines(buf, 3, 3, false, { "╭──", "10 + 20", "╰──" })
    local cleanup_cell = session.model:cell_at_row(4).id
    vim.api.nvim_set_current_buf(buf)
    vim.api.nvim_win_set_cursor(0, { 5, 0 })
    session.editing.cellmode:set(true)
    vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("<CR>", true, false, true), "xt", false)
    session.editing.cellmode:set(false)
    wait_for(function() return session.presentation:buffer_for_cell(cleanup_cell) ~= nil end, "Enter did not execute cleanup test cell")
    assert(not vim.api.nvim_buf_is_valid(previous_output), "Enter retained previous completed output")
    jusi.close(buf, 4)
    ...
    assert(session.lifecycle.parked[cell_id])
    local parked_mark = vim.api.nvim_buf_get_extmark_by_id(buf, session.marks.namespace,
      session.marks.records[cell_id].mark, { details = true })
    assert(parked_mark[3].virt_text[1][1]:find('~', 1, true), 'parking did not add its tilde')
    session.editing.cellmode:set(false)
    jusi.execute(buf, 4)
