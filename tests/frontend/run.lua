...
local interrupt = read_json("protocol/fixtures/v1/valid/interrupt-execution.json")
local interrupt_ok, interrupt_error = protocol.validate_command(interrupt, "interrupt")
assert(interrupt_ok, interrupt_error)
local invalid_interrupt = read_json("protocol/fixtures/v1/invalid/interrupt-missing-execution-id.json")
assert(not protocol.validate_command(invalid_interrupt, "interrupt"), "interrupt requires execution identity")
...
local active_health = read_json("protocol/fixtures/v1/valid/health-active-execution.json")
assert(protocol.validate_health_response(active_health))
...
local missing_executions_health = read_json("protocol/fixtures/v1/invalid/health-missing-executions.json")
assert(not protocol.validate_health_response(missing_executions_health), "authoritative health must include active executions")
