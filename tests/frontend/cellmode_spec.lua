...
function M.run()
  ...
  assert(marks[1][4].hl_group == 'JusiCellIdle')
  assert(marks[1][4].virt_text[1][1] == '╔══' and marks[1][4].virt_text_pos == 'overlay')
  assert(marks[2][4].virt_text[1][1] == '╚══')
  -- A missed InsertLeave must not leave working Normal mappings with stale visuals.
  vim.api.nvim_exec_autocmds('InsertEnter', { buffer = buf })
  assert(mode.enabled and not vim.b.jusi_cell_mode_active)
  keys('<Space><Space>')
  assert(mode.enabled and vim.b.jusi_cell_mode_active)
  assert(require('jusi.statusline').render():find('mode:cell', 1, true))
  marks = vim.api.nvim_buf_get_extmarks(buf, editor.offline_marks.namespace, 0, -1, { details = true })
  assert(marks[1][4].virt_text[1][1] == '╔══')
