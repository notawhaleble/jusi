...
def deliver(content: dict[str, Any], *, socket_path: str | None = None) -> dict[str, str]:
    ...
    request = {"protocol_version": 1, "kind": "application.editor_action", "request_id": request_id, "content": content}
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if len(data) > 524288:
            raise EditorDeliveryError("File exceeds the 512 KiB text limit")
        open_text(data.decode("utf-8"), name=path.name, filetype=args.filetype)
